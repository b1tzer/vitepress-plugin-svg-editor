...
export async function waitForSvgContainer(
  page: Page,
  svgIndex = 0,
  timeout = 15000
): Promise<void> {
 ...
 */
export async function navigateAndOpenEditor(page: Page, url: string, svgIndex = 0): Promise<void> {
...
export async function createRects(page: Page, n: number, pfx: string) {
  ...
  return page.evaluate(
    ({ n, pfx, colors }: any) => {
      ...
      // 清理旧对象
      c.getObjects()
        .filter((o: any) => o.id?.startsWith(pfx))
...
export async function addTestShapes(page: Page) {
  const rectIdx = await addRect(page, {
    left: 100,
    top: 100,
    fill: '#2196F3',
    width: 150,
