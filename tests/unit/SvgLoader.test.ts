...
describe('SvgLoader', () => {
  ...
  it('应处理无 viewBox 的 SVG', () => {
    const noViewBox = '<svg xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="40"/></svg>'
  ...
  it('应保持 SVG 结构完整性', () => {
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 300"><g><path d="M10 10 L90 90"/><text x="20" y="50">Hello</text></g></svg>'
  ...
  it('应移除内联事件处理器（onclick/onload 等）', () => {
    const malicious = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect onclick="alert(1)" onload="bad()"/></svg>'
    const result = loader.load(malicious, 'light')
  ...
  it('应移除 CSS @import 外部引用防止 CSS 注入', () => {
    const malicious = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><style>@import url(http://evil.com/bad.css);</style><rect/></svg>'
    const result = loader.load(malicious, 'light')
