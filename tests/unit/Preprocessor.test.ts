...
describe('preprocessSvg', () => {
  ...
  it('应解析 marker 属性顺序打乱的 SVG', () => {
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><defs><marker refY="4" id="arrow" refX="4" markerHeight="8" markerWidth="10"><polygon fill="#000" points="0,0 10,4 0,8"/></marker></defs><line x1="0" y1="0" x2="100" y2="0" stroke="#000" marker-end="url(#arrow)"/></svg>'
    const result = preprocessSvg(svg, 'light')
    expect(result.svg).toContain('<polygon')
    expect(result.svg).not.toContain('marker-end')
  })
  ...
  it('不应误删 fill-opacity 非 0 的 rect', () => {
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect x="0" y="0" width="100" height="100" style="stroke:none;fill-opacity:0.5" fill="#FF0000"/></svg>'
    const result = preprocessSvg(svg, 'light')
  ...
  it('应移除 fill-opacity 精确为 0 的不可见占位 rect', () => {
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect x="0" y="0" width="100" height="100" style="stroke:none;fill-opacity:0"/><rect x="10" y="10" width="20" height="20" fill="#00FF00"/></svg>'
    const result = preprocessSvg(svg, 'light')
