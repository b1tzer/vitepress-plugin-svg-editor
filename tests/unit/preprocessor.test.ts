...
describe('preprocessSvg', () => {
  it('应正确提取 viewBox 和尺寸', () => {
    const result = preprocessSvg(baseSvg)
  ...
  it('应解析带 fallback 的外部 CSS 变量为 fallback 值', () => {
    // 例如 VitePress 的 var(--vp-c-brand-1, #2563eb)，带兜底值应取 fallback，
  ...
  it('不带 fallback 的 var(--xxx) 保持原样（不再做静态色板解析）', () => {
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="var(--diagram-surface-1)"/></svg>'
    const result = preprocessSvg(svg)
    // 静态色板已删除，不带 fallback 的变量不再被替换
    expect(result.svg).toContain('var(--diagram-surface-1)')
  })
