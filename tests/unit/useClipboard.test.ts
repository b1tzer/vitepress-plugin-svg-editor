...
vi.mock('fabric', () => ({
  ActiveSelection: vi.fn((objs: unknown[]) => ({ type: 'activeselection', _objects: objs })),
}))
...
describe('useClipboard', () => {
  ...
  it('copy 无选中对象时不应报错', async () => {
    ...
    await expect(cb.copy()).resolves.toBeUndefined()
  })

  it('copy 单个对象时应调用 clone', async () => {
    const obj = { type: 'rect', clone: vi.fn().mockResolvedValue({ type: 'rect' }) }
    canvas.getActiveObject.mockReturnValue(obj)
    await cb.copy()
    expect(obj.clone).toHaveBeenCalled()
  })

  it('copy 多选时应保存子对象', async () => {
  ...
  it('paste 单个对象时应 add + setActiveObject + commit', async () => {
    ...
    const clipboardClone = {
      ...
      top: 10,
      set: vi.fn(),
      clone: vi.fn().mockResolvedValue(finalClone),
    }
    canvas.getActiveObject.mockReturnValue({
      type: 'rect',
      clone: vi.fn().mockResolvedValue(clipboardClone),
    })
    await cb.copy()
  ...
  it('paste 多对象时应创建 ActiveSelection', async () => {
    const cloneResult1 = {
      ...
      top: 0,
      set: vi.fn(),
      clone: vi.fn().mockResolvedValue({ type: 'rect', left: 0, top: 0, set: vi.fn() }),
    }
    const cloneResult2 = {
      type: 'circle',
      left: 0,
      top: 0,
      set: vi.fn(),
      clone: vi.fn().mockResolvedValue({ type: 'circle', left: 0, top: 0, set: vi.fn() }),
    }
    const sel = {
