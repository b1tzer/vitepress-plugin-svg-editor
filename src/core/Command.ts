...
export class AddCommand implements ICommand {
  private _obj: FabricObject
  private _canvas: any  // eslint-disable-line @typescript-eslint/no-explicit-any
  private _index: number = -1

  /**
   * @param obj    要添加的 Fabric 对象
   * @param canvas Fabric.js 画布实例（标注为 any 避免循环依赖）
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(obj: FabricObject, canvas: any) {
    this._obj = obj
    this._canvas = canvas
  }

  execute(): void {
    this._canvas.add(this._obj)
    this._index = this._canvas.getObjects().length - 1
    this._canvas.requestRenderAll()
  }

  undo(): void {
    if (this._index >= 0) {
      this._canvas.remove(this._obj)
      this._canvas.requestRenderAll()
    }
  }

  getLabel(): string {
    return `添加${this._getTypeName()}`
  }
