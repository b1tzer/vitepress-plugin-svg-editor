...
watch(
  () => props.svgWidth,
  (v) => {
    editingWidth.value = v
  }
)
watch(
  () => props.svgHeight,
  (v) => {
    editingHeight.value = v
  }
)
...
<template>
  <header class="editor-toolbar" :class="isLight ? 'toolbar-light' : 'toolbar-dark'">
    ...
    <div class="toolbar-left">
      ...
      <span class="sep" />
      <button @click="emit('copy')" data-tip="复制 Ctrl+C" aria-label="复制 Ctrl+C">
        <span v-html="ICONS.copy"></span>
      </button>
      <button @click="emit('paste')" data-tip="粘贴 Ctrl+V" aria-label="粘贴 Ctrl+V">
        <span v-html="ICONS.paste"></span>
      </button>
      <button @click="emit('delete')" data-tip="删除 Delete" aria-label="删除 Delete">
        <span v-html="ICONS.trash"></span>
      </button>
      <span class="sep" />
      <button @click="emit('zoomOut')" data-tip="缩小 Ctrl+-" aria-label="缩小 Ctrl+-">
        <span v-html="ICONS.zoomOut"></span>
      </button>
      <span
        class="zoom-badge"
        @click="emit('zoomFit')"
        ...
        role="button"
        tabindex="0"
        aria-label="缩放比例 点击重置"
      >
        {{ zoomLevel }}%
      </span>
      <button @click="emit('zoomIn')" data-tip="放大 Ctrl+=" aria-label="放大 Ctrl+=">
        <span v-html="ICONS.zoomIn"></span>
      </button>
      <button @click="emit('zoomFit')" data-tip="适应画布 Ctrl+0" aria-label="适应画布 Ctrl+0">
        <span v-html="ICONS.zoomFit"></span>
      </button>
    </div>
    ...
    <div class="toolbar-right">
      ...
      <button class="btn-close" @click="emit('close')" data-tip="关闭 Esc" aria-label="关闭 Esc">
...
.editor-toolbar button span svg {
  width: 15px;
  height: 15px;
  stroke-width: 1.8;
}
...
.toolbar-dark button:hover:not(:disabled) {
  background: rgba(255, 255, 255, 0.08);
  color: #e0e0e0;
}
.toolbar-dark button:active:not(:disabled) {
...
.toolbar-light button:hover:not(:disabled) {
  background: rgba(0, 0, 0, 0.06);
  color: #222;
}
.toolbar-light button:active:not(:disabled) {
  transform: scale(0.95);
}
...
.toolbar-dark .zoom-badge {
  color: #aaa;
  background: rgba(255, 255, 255, 0.04);
}
.toolbar-dark .zoom-badge:hover {
  background: rgba(255, 255, 255, 0.08);
  color: #e0e0e0;
}
.toolbar-light .zoom-badge {
  color: #555;
  background: rgba(0, 0, 0, 0.04);
}
.toolbar-light .zoom-badge:hover {
...
.toolbar-dark .info-tag {
  color: #777;
  background: rgba(255, 255, 255, 0.03);
}
.toolbar-dark .info-selection {
  color: #888;
}
.toolbar-light .info-tag {
  color: #888;
  background: rgba(0, 0, 0, 0.03);
}
.toolbar-light .info-selection {
  color: #555;
}
...
.toolbar-dark .size-input {
  color: #aaa;
  background: rgba(255, 255, 255, 0.04);
}
.toolbar-dark .size-input:focus {
  border-color: rgba(255, 255, 255, 0.2);
  background: rgba(255, 255, 255, 0.08);
  color: #e0e0e0;
}
.toolbar-light .size-input {
  color: #555;
  background: rgba(0, 0, 0, 0.04);
}
.toolbar-light .size-input:focus {
  border-color: rgba(0, 0, 0, 0.15);
  background: rgba(0, 0, 0, 0.06);
  color: #222;
}
.toolbar-dark .size-sep,
.toolbar-dark .size-unit {
  color: #666;
}
...
.toolbar-dark [data-tip]::after {
  background: rgba(255, 255, 255, 0.9);
  color: #222;
}
.toolbar-light [data-tip]::after {
