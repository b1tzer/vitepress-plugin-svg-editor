...
- ~~暗色/亮色模式切换时，用户需要提供两套映射~~（**已过时**：v0.3.0 起未命中语义色板的自定义色由 OKLCH 自适应算法自动派生暗色，无需用户再提供两套映射，见 ADR-009）
...
## ADR-009：语义化颜色 ID + OKLCH 自适应颜色算法

**日期**：2026-08-19
**状态**：✅ 已确定

**决策**：将 SVG 颜色处理升级为「语义 token 优先 + OKLCH 自适应兜底」两级体系，替代 ADR-006 中「用户提供两套映射」的假设。

**理由**：

1. ADR-006 假设「暗色/亮色切换时用户提供两套映射」，但实际自定义色（用户手绘、外部导入的裸 hex）数量不可枚举，无法预置映射。
2. 28 组 `--diagram-*` 语义色板只覆盖内置配色，自定义色在暗色下原样保留会导致对比度失衡。
3. OKLCH 是感知均匀色彩空间（Tailwind v4、shadcn/ui、Figma 均默认采用），在 OKLCH 中翻转亮度（`L' = 1 - L`）可保色相、保饱和度，过渡自然不发灰发紫。

**方案**：

- **语义 token 优先**：导入时给对象打 `fillVar`/`strokeVar` 语义 ID，切换主题时按变量名精确取色（消除撞色歧义）；导出时把「仍保持语义」的对象还原为 `var(--diagram-*)`。
- **OKLCH 自适应兜底**：未命中语义色板的颜色在 OKLCH 空间做亮度翻转。高饱和边界色（如 `#FF0000`）翻转后会被推出 sRGB 色域，因此用「双向记忆化映射」（`lightToDarkCache` / `darkToLightCache`）保证「亮→暗→亮」精确等幂恢复，而非依赖自逆函数。
- **保存归一化**：保存时若处于暗色模式，借 `darkToLightCache` 反向映射把非语义色归一化回亮色真值，落盘 SVG 永远保存亮色语义。
- **展示层派生**：`SvgDiagram` 用 `v-html` 内联 SVG，在页面切换 `.dark` 时对裸 hex 运行时派生暗色，`var(--diagram-*)` 交由 CSS 自动切换。

**替代方案**：

- A. 页面侧 `filter: invert(1) hue-rotate(180deg)`：零改 SVG，但多色图色相失真（红变青）→ ❌
- B. 维护亮/暗两张 SVG 资源：效果最准但维护成本翻倍 → ❌
- C. 引入 `culori` 等第三方颜色库：功能齐全但项目 `dependencies` 极克制（仅 dompurify），零依赖手写矩阵更可控 → 采用手写

**后果**：

- 需要手写 sRGB ↔ OKLCH 转换矩阵（约 150 行，零依赖）。
- 需要处理 sRGB 色域裁剪问题（高饱和边界色靠记忆化兜底）。
- 需要保证「编辑器切换」与「展示层派生」两处颜色逻辑的一致（共享 `adaptiveColor` + `LIGHT_TO_DARK` 纯函数）。
- 需要处理真实浏览器把 inline style 的 `#FFFFFF` 规范化为 `rgb()` 的差异（从 `getAttribute('style')` 原始字符串提取 hex）。
